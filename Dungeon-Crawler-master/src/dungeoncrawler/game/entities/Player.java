...
public class Player extends Entity {
	...
	public void decreaseTime() {
		...
		if (this.critFlashTimer > 0) this.critFlashTimer--;
	}
	...
	public void attack() {
		...
		if (now - lastAttackTime < 300) {
			criticalHit = true;
			critFlashTimer = 5; // The flash lasts 5 frames
		} else {
			criticalHit = false;
		}
	...
	public void render(Graphics graphics) {
		...
		if (critFlashTimer > 0) {
			graphics.setColor(new Color(255, 255, 0, 128)); // translucent yellow circle
			graphics.fillOval(x - 8, y - 8, width + 16, height + 16); // flash around the player
		}

		// Walking animation (if the player is moving and not attacking)
		...
		// Render the player
		graphics.drawImage(
				Resources.TEXTURES.get(entityID + animationFrame),
				super.x, super.y,
	...
	private void restartGame() {
		System.out.println("Player died. Restarting game...");
	...
	public void damage(int amount) {
		if (this.damageTime == 0) {
			...
			if (this.hp == 0 && onDeathCallback != null) {
				onDeathCallback.run();
			}
		}
	}

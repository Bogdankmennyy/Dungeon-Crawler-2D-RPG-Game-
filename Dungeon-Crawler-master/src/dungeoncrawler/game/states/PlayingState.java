...
public class PlayingState extends GameState {
	...
	private int deathDelay = -1;



	protected PlayingState(GameStateManager manager) {
		super(manager);
		this.manager = manager; //  save for restart

		this.generator = new LevelGenerator();
		this.player = new Player();

		this.player.setOnDeathCallback(() -> {
			this.deathDelay = 120; // ~2 seconds if loop is called 60 times/sec
		});
	...
	protected void loop() {
		...
		if (deathDelay == 0) {
			...
			this.manager.stackState(new PlayingState(this.manager));
			return;
		}
	...
	private void playerAttacks() {
		...
		for(int i=0;i<this.world.getRoom().getEnemies().size();i++) {
			...
			if(this.world.getRoom().getEnemies().get(i).intersects(this.player.getAttackBox())) {
				int damage = player.isCriticalHit() ? 6 : 3;
				this.world.getRoom().getEnemies().get(i).damage(damage, this.player.getFacing());
